...
const SPORTS = [
  ...
  { id: "basketball", icon: sportBasketball, emoji: "🏀", name: "籃球", nameEn: "BASKETBALL", accent: "#1F5BB1", score: "24", features: ["節次管理", "24秒進攻計時", "犯規記錄", "球權顯示"] },
...
function SportCard({ sport, onClick }) {
  ...
  return (
    <div onClick={onClick}
      ...
      style={{
        position: "relative", background: hovered ? "#EEF8FF" : "#FFFFFF",
        border: `1.5px solid ${hovered ? sport.accent : "#DCE8E2"}`,
      ...
      <div>
        <div style={{ fontSize: 22, fontWeight: 900, color: "#1A1A1A", letterSpacing: 1, lineHeight: 1 }}>{sport.name}</div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginTop: "auto" }}>
        {sport.features.map(f => (
          <span key={f} style={{ fontSize: 10, color: "#666", background: "#F0F5F2", border: "1px solid #DCE8E2", borderRadius: 6, padding: "2px 8px" }}>{f}</span>
        ))}
      </div>
...
export default function Homepage() {
  ...
  return (
    <div style={{
      minHeight: "100vh", background: "#F0F5F2",
      ...
      {/* NAV */}
      <nav style={{
        height: 52, flexShrink: 0, borderBottom: "1px solid #EEF8FF",
        display: "flex", alignItems: "center", justifyContent: "space-between",
        padding: "0 28px", background: "#F0F5F2",
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <img src={logo} alt="KEEGOING" style={{ height: 34, width: "auto" }} />
          <span style={{ fontSize: 15, fontWeight: 900, letterSpacing: 2, color: "#1A1A1A" }}>KEEGOING</span>
        </div>

        {/* Tab switcher */}
        <div style={{
          display: "flex", background: "#EEF8FF", borderRadius: 10,
          padding: 3, gap: 2,
        }}>
          {[
            ...
            <button key={t.key} onClick={() => t.key === "player" ? navigate("/player") : setTab(t.key)} style={{
              ...
              background: tab === t.key ? "#1F5BB1" : "transparent",
      ...
      {/* HERO */}
      <div style={{
        padding: "48px 28px 40px", borderBottom: "1px solid #C7D8CF",
        position: "relative", overflow: "hidden",
      }}>
        <div style={{
          position: "absolute", top: -20, right: 28,
          fontSize: "clamp(80px, 18vw, 180px)", fontWeight: 900,
          fontFamily: "'Courier New', monospace", color: "#1F5BB1", opacity: 0.05,
        ...
        <h1 style={{
          ...
          color: "#1A1A1A", margin: "0 0 14px", letterSpacing: -1,
          fontFamily: "'MantouSans', 'Noto Sans TC', sans-serif",
        }}>
          每一場對決，都值得被記錄。<br/>
          <span style={{ color: "#1F5BB1" }}>現在就揪朋友，開始比賽！</span>
        </h1>
      ...
      <div onClick={() => navigate("/player")} style={{
        ...
        border: "1px solid #C7D8CF", borderRadius: 14,
        padding: "18px 24px", cursor: "pointer",
        display: "flex", alignItems: "center", justifyContent: "space-between",
        transition: "border-color .2s",
      }}
        onMouseEnter={e => e.currentTarget.style.borderColor = "#1F5BB1"}
        onMouseLeave={e => e.currentTarget.style.borderColor = "#C7D8CF"}
      >
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{
            width: 44, height: 44, borderRadius: 12, background: "#1F5BB122",
            border: "1px solid #1F5BB144",
            display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 22,
          }}>👤</div>
          <div>
            <div style={{ fontSize: 14, fontWeight: 800, color: "#1A1A1A", marginBottom: 4 }}>球員版</div>
      ...
      {/* FOOTER */}
      <div style={{
        borderTop: "1px solid #C7D8CF", padding: "14px 28px",
        display: "flex", alignItems: "center", justifyContent: "space-between",
      }}>
        <span style={{ fontSize: 10, color: "#333", letterSpacing: 1 }}>KEEGOING © 2026</span>
        <span style={{ fontSize: 10, color: "#C7D8CF", letterSpacing: 1 }}>廣告支援營運</span>
      </div>
    </div>
  );
}
